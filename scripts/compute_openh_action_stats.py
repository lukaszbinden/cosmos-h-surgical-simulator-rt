...
def process_single_dataset(
    ...
    timestep_interval_override: int | None = None,
):
    ...
    Args:
        ...
        timestep_interval_override: If given (int > 0), override the
            ``timestep_interval`` value read from EMBODIMENT_REGISTRY for
            this run. Useful to experiment with a different effective training
            rate *without* editing ``groot_configs.py``. IMPORTANT: if you use
            this, training must run with the same stride, otherwise
            ``stats_cosmos.json`` will not match the distribution the model
            sees.
    """
    ...
    registry_timestep_interval = reg["timestep_interval"]
    if timestep_interval_override is not None:
        if timestep_interval_override < 1:
            raise ValueError(
                f"--timestep-interval must be >= 1, got {timestep_interval_override}"
            )
        timestep_interval = int(timestep_interval_override)
    else:
        timestep_interval = registry_timestep_interval

    print("=" * 80)
    print(f"COMPUTING OPEN-H ACTION STATS — {dataset_path.name}")
    print(f"  embodiment: {embodiment}")
    print(f"  num_frames: {num_frames}")
    if timestep_interval_override is not None and timestep_interval_override != registry_timestep_interval:
        print(
            f"  timestep_interval: {timestep_interval} "
            f"(OVERRIDE; registry default for '{embodiment}' is {registry_timestep_interval})"
        )
    else:
        print(f"  timestep_interval: {timestep_interval}  (from EMBODIMENT_REGISTRY)")
    ...
    # ----------------------------------------------------------------
    # Stamp provenance metadata for downstream guards.
    # ----------------------------------------------------------------
    # We record the exact ``timestep_interval`` used to compute these
    # statistics as a top-level integer so ``LeRobotSingleDataset`` can
    # assert it still matches EMBODIMENT_REGISTRY at training time.
    # Since all real stat entries are per-key dicts (``"action.psm1_pose"``
    # etc.), an int at the top level never collides with a stat, and the
    # existing validation loop in ``dataset.py`` (``if isinstance(stat, int):
    # continue``) already skips it.
    stats["timestep_interval"] = int(timestep_interval)
    ...
    for i, (dp, emb, excl) in enumerate(jobs, 1):
        ...
        try:
            ok = process_single_dataset(
                ...
                timestep_interval_override=args.timestep_interval,
            )
...
def main():
    ...
    parser.add_argument("--timestep-interval", type=int, default=None,
                        help="Override the 'timestep_interval' (action stride) that is otherwise "
                             "read from EMBODIMENT_REGISTRY in groot_configs.py. Useful when you "
                             "want to compute stats for a different effective training rate than "
                             "the registry default (e.g. use 3 instead of 5 on a 30Hz dVRK dataset "
                             "to target 10 Hz effective). IMPORTANT: if you override this here, "
                             "the training pipeline MUST use the same value or the resulting "
                             "stats_cosmos.json will not match the distribution the model sees.")

...
# JHU dVRK Mono warmup cache (Phase 0 output of the SF pipeline). The cache is
# generated by ``inference_jhu_dvrk_warmup.py`` over the 9-dataset JHU dVRK
# mixture (frame-proportional weighting via JHU_DVRK_MONO_FINETUNE_TRAIN_DATASET_SPECS)
# at 288x512 resolution using the annealed teacher checkpoint. Action tensors
# stored in actions/<idx>.json are 44-D (zero-padded by MixedLeRobotDataset).
dataset_jhu_dvrk_mono_warmup = L(ActionDatasetSFWarmup)(
    data_path="datasets/jhu_dvrk_mono_warmup_4step",
    cr1_embeddings_path="cr1_empty_string_text_embeddings.pt",
)
...
def register_interactive_data():
    ...
    cs.store(
        group="data_train",
        package="dataloader_train",
        name="jhu_dvrk_mono_warmup",
        node=make_dataloader(dataset_jhu_dvrk_mono_warmup),
    )
    cs.store(
        group="data_val",
